...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadRequirements(); loadUsers(); loadSettings(); loadPositions();
    connectWebSocket(); startPolling(); renderStoredNotices();
});

// ===== 提醒持久化 =====
function readNotices() { try { return JSON.parse(localStorage.getItem(NOTICE_KEY) || '[]'); } catch (e) { return []; } }
function writeNotices(arr) { try { localStorage.setItem(NOTICE_KEY, JSON.stringify(arr.slice(0, 30))); } catch (e) {} }
...
function renderStoredNotices() {
    ...
    fresh.forEach(n => { if (n.type === 'new_requirement') appendNewRequirementCard(n, n._id); else appendFollowUpCard(n, n._id); });
    newReqBadge = fresh.length; updateTaskBadge();
}
function dismissNotice(noticeId) {
    removeNoticeFromStore(noticeId);
    const el = document.getElementById(noticeId);
    if (el) el.remove();
    newReqBadge = Math.max(0, newReqBadge - 1); updateTaskBadge();
}
    ...
    try {
        ...
        <div style="margin-bottom:16px;">
            ...
            ${req.solution ? `<div style="background:#d1fae5;padding:12px 16px;border-radius:8px;margin-bottom:12px;">
                <strong>✅ 解决方案:</strong>
                <p style="margin-top:6px;white-space:pre-wrap;">${escapeHtml(req.solution)}</p></div>` : ''}
        </div>
        ...
        if (visibleMsgs.length > 0) {
            ...
            for (const msg of visibleMsgs) {
                const attach = msg.attachment_path ? `<a href="${(window.BACKEND_ORIGIN||'')}/uploads/${encodeURIComponent(msg.attachment_path)}" download="${escapeHtml(msg.attachment_name)}" style="color:#2563eb;margin-left:6px;">📎 ${escapeHtml(msg.attachment_name)}</a>` : '';
                html += `
        ...
        } else { html += `<div style="color:#94a3b8;padding:8px 0;">暂无消息</div>`; }
        html += `</div></div>
...
function showResolveForm() {
    ...
    <div style="background:#f8fafc;padding:16px;border-radius:8px;margin:8px 0;">
        ...
        <div class="form-group"><label>备注（可选）</label>
            <input type="text" id="resolveRemark" placeholder="补充说明"></div>
...
async function askAssistant(query) {
    ...
    container.insertAdjacentHTML('beforeend', `
        <div class="message user">
            ...
            <div class="bubble"><span class="msg-sender">我</span>
    ...
    try {
        ...
        if (el) {
            el.querySelector('.bubble').innerHTML = `<span class="msg-sender">AI助手</span>
                ${escapeHtml(res.answer || '（AI未返回内容）').replace(/\n/g, '<br>')}
                <span class="msg-time">${formatTime(new Date().toISOString())}</span>`;
            scrollToBottom('assistantMessages');
        }
    } catch (e) {
        const el = document.getElementById(loadingId);
        if (el) el.querySelector('.bubble').innerHTML =
            `<span class="msg-sender">AI助手</span><span style="color:#e74c3c;">处理失败：${escapeHtml(e.message)}</span>`;
    }
}
...
function buildLineChartSVG(labels, s1, s2, name1, name2, c1, c2) {
    ...
    let dots = '', vals = '';
    pts1.forEach((p,i)=>{ dots += `<circle cx="${p[0]}" cy="${p[1]}" r="3.5" fill="#fff" stroke="${c1}" stroke-width="2"><title>${name1} ${s1[i]}</title></circle>`;
        vals += `<text x="${p[0]}" y="${p[1]-9}" font-size="10" fill="${c1}" text-anchor="middle">${s1[i]||''}</text>`; });
    pts2.forEach((p,i)=>{ dots += `<circle cx="${p[0]}" cy="${p[1]}" r="3.5" fill="#fff" stroke="${c2}" stroke-width="2"><title>${name2} ${s2[i]}</title></circle>`; });
...
function renderTrend(reqs) {
    ...
    return `<div style="display:flex;align-items:flex-end;gap:10px;height:150px;">` + labels.map((lb,i)=>`
...
function renderReport(data, container) {
    ...
    const todoHtml = todo.length ? todo.map(r=>`
        <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:13px;cursor:pointer;" onclick="showRequirementDetail(${r.id})">
            ...
            <span style="color:#94a3b8;">${escapeHtml(r.username || ('用户 #'+r.user_id))} · ${escapeHtml(r.status)}</span></div>`).join('')
        : '<div style="color:#94a3b8;padding:8px 0;">🎉 暂无待办，全部解决！</div>';
    const statusMap = { '待处理':'pending','处理中':'processing','待反馈':'waiting','已解决':'resolved' };
...
async function exportReport(period) {
    ...
    if (period === 'monthly') {
        ...
        filename = `需求月报_${y}-${String(m+1).padStart(2,'0')}`; unit = 'day';
    ...
    const s1 = buckets.map(b => submitted.filter(r => keyOf(r,'created_at') === b.key).length);
    const s2 = buckets.map(b => solvedInP.filter(r => keyOf(r,'updated_at') === b.key).length);
    ...
    const chart = buckets.length > 1 ? buildLineChartSVG(buckets.map(b=>b.label), s1, s2, '提交', '解决', '#2563eb', '#22c55e') : '<p class="mut">周期内数据点不足，暂无趋势图</p>';
    const html = `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="utf-8"><title>${filename}</title><style>
        ...
        th,td{border-bottom:1px solid #e9eef5;padding:7px 8px;text-align:left;} th{background:#f8fafc;}
        .mut{color:#94a3b8;} @media print{body{padding:0;}}
    </style></head><body>
...
function renderPositionDropdown(names) {
    ...
    dropdown.innerHTML = names.map(name => `
        <div style="padding:8px 14px;cursor:pointer;border-bottom:1px solid #f1f5f9;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='white'" onclick="selectPosition('${name}')">${escapeHtml(name)}</div>`).join('');
}
...
function appendNewRequirementCard(data, noticeId) {
    ...
    <div class="message notice" id="${noticeId}">
        <div class="avatar">🔔</div>
        <div class="bubble"><span class="msg-sender">新任务提醒
            <span style="float:right;cursor:pointer;" title="关闭" onclick="dismissNotice('${noticeId}')">✕</span></span>
...
function appendFollowUpCard(data, noticeId) {
    ...
    <div class="message notice" id="${noticeId}">
        <div class="avatar">💬</div>
        <div class="bubble"><span class="msg-sender">子节点补充消息
            <span style="float:right;cursor:pointer;" title="关闭" onclick="dismissNotice('${noticeId}')">✕</span></span>
    ...
    if (form) {
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                closeModal(); loadUsers(); loadPositions();
        ...
        box.innerHTML = docs.map(d => `
            <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;border-bottom:1px solid #f1f5f9;">
                <div><strong>${escapeHtml(d.title)}</strong>
                    <span style="color:#94a3b8;font-size:12px;margin-left:8px;">${d.chunks} 个片段 · ${escapeHtml(d.source_name || '手输入')}</span></div>
...
async function uploadKnowledgeFile(input) {
    const f = input.files[0]; input.value = '';
    if (!f) return;
    showToast('解析并向量化中，请稍候...', 'info');
    try {
        const r = await API.uploadKnowledgeDoc(f);
        showToast(`已入库「${r.title}」（${r.chunks}个片段）`, 'success');
        loadKnowledge();
...
async function deleteKnowledgeDoc(docId) {
    ...
    try { await API.deleteKnowledgeDoc(docId); showToast('已删除', 'success'); loadKnowledge(); }
    catch (e) { showToast('删除失败: ' + e.message, 'error'); }
}
